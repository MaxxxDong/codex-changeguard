...
Public seams: `changeguard platform-status [--probe-host=…] [--receipt=<path>] [--plan] [--adapter=<id>]`, MCP `changeguard_platform_status` (unified: Ticket 13 macOS capability fields, Ticket 14 Windows evaluation under `status`, Ticket 15 capability matrix under `reports` / `default_status`). Schema: `schemas/platform-support-receipt.schema.json` is a oneOf of macOS harness receipts and Windows 11 support receipts; Ticket 15 lightweight `SupportReceipt` is a separate contract (`schemas/platform-capability.schema.json`) and never upgrades production Full. The Windows real-machine runner entry (`src/platform/windows/runner.ts`) validates receipts only — it never executes Codex binaries, never writes WindowsApps/Program Files/registry policy, never elevates, and never seals a live witness. Synthetic fixtures under `fixtures/windows11/receipts/` can only support PREVIEW. Runtime receipt parse fail-closes on unknown extra keys (top-level, scenarios, attestation) to match `additionalProperties: false`. Public recovery CLI/MCP paths bind host-derived capability via `resolvePublicRepairCapability(env, platform, exactTarget)`: the internal env flag `CHANGEGUARD_INTERNAL_FIXTURE_SEAM=1` is **not** authorization by itself. Isolated PREVIEW writes require both that operator/harness-controlled env **and** exact-target disposable isolation proof (`proveIsolatedFixtureTarget`: ordinary non-symlink directory that is a **strict descendant** of trusted OS temp roots — never equal to a configured/canonical trusted temp root such as `os.tmpdir()` / `TMPDIR` / `TMP` / `TEMP` — not active `~/.codex`, not protected/system/host-mount paths — reusing path-safety + disposable-target invariants; root-equality refusal is repair-seam-specific and does not weaken harness `assertDisposableTarget` deny-root semantics). In-repo fixtures, HOME directories, exact shared temp roots, symlink escapes, and `/mnt/<drive>` remain `production_unknown` / `WRITE_DISABLED` even with the env set. External JSON cannot inject PREVIEW or enable writes. Linux/WSL remain Limited with writes disabled until a real-machine receipt exists.
