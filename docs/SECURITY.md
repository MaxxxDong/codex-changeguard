...
- **System enumeration bounds:** production adapter only probes registered Desktop / PATH / package-root / MSIX / native Linux / WSL candidates under hard caps; missing permissions or metadata become `unavailable`, never broad home crawls or execution
- **Linux/WSL capability (Ticket 15):** unverified adapters default to `READ_ONLY` or `LIMITED` with mutation disabled; native Linux CLI uses `install_source` path/package (never `wsl`); WSL and Windows host identities never collapse; `/mnt/c` host mounts are refused as Linux trusted roots
- **Enterprise managed policy:** managed/admin-owned/signed/permission-bound targets return `ADMIN_ACTION_REQUIRED` with IT Handoff (minimal digests, requested action, risk, rollback, official reference) — never sudo/chmod/bypass/elevation guidance, never absolute paths or secrets
- **Network/proxy/cert/SSO/firewall:** non-destructive comparison of orchestrator-supplied observations only; production platform modules open no sockets and change no system settings
