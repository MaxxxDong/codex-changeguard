...
- **Internal fixture seam is not authorization:** `CHANGEGUARD_INTERNAL_FIXTURE_SEAM=1` only marks a harness/operator process. Public CLI/MCP repair-preview and repair-apply still bind the **exact target** through `resolvePublicRepairCapability`; isolated PREVIEW requires concurrent disposable-target proof (OS temp isolation root, ordinary non-symlink directory, no active `~/.codex` / protected / host-mount path). Env alone, in-repo fixtures, HOME paths, and symlink aliases remain fail-closed (`WRITE_DISABLED` / `production_unknown`)
