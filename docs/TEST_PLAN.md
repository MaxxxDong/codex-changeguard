...
| Local artifact baseline/diff | v1→v2 migration honesty (no invented history), first baseline ≠ content_changed, unchanged silent SessionStart, exact hash change without version change, content+gap and gap-only classifications, added/removed, stable ordering, symlink refusal, oversize gap (no truncated hash), binary bytes, Unicode alias / path redaction, multi-instance isolation, SessionStart path-free artifact status/counts/keys + gap_changed count, pure-artifact headline (not version), mid-file streaming time budget → `time_budget_exceeded` (no truncated digest), repeated identical `time_budget_exceeded` baselines stay non-silent partial/unavailable (not unchanged), incomplete/unavailable SessionStart headlines, corrupt baseline/overall digests and binding mismatches fail closed |
