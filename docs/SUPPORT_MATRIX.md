...
| --- | --- | --- | --- | --- | --- |
| macOS | Full (first full path) | **Full** (receipt-scoped on verified real-machine harness for this host; Ticket 13 `LOCAL_COMPLETE`) | Full only when real-machine receipt has every required scenario `pass` **and** current-process live harness witness validation succeeds; external/CLI/MCP/arbitrary JSON alone is at most **Preview** | Namespaced adapter in `src/platform/macos/` | Exact Desktop paths include official **ChatGPT.app** (`…/Contents/Resources/codex`) and legacy **Codex.app**; same path Desktop+PATH dedupes to `desktop_bundled` with `plist_metadata`. Disposable temp fixtures only; never active `~/.codex` (logical or realpath; symlink fail-closed). Not a universal macOS/Codex guarantee |
